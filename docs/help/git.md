...
How strata connects
- strata calls `git` commands from the process environment. Ensure the user or service account running the platform has appropriate credentials (SSH key or PAT) and that the shell environment exposes any custom `GIT_*` variables needed for authentication.
