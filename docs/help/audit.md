...
| `cost.threshold_exceeded` | on      | Domain     | `…strata.cost.threshold_exceeded` | `cost show` only (not `cost diff`/`deploy run`'s auto-diff — neither records history); requires `spec.cost.history.alert` |
