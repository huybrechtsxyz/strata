...
| 5 | Lifecycle hierarchy ambiguity (workspace/namespace/module precedence undocumented) | Still not addressed by any ADR. Captured and scoped in [lifecycle.md](lifecycle.md) (open question 2), which also records that v2 executes lifecycle *nowhere* — so precedence is currently unobservable rather than merely undocumented | Open |
...
- Lifecycle hierarchy precedence (Architectural Issue 5) — no ADR yet; see [lifecycle.md](lifecycle.md) for the full open-question set it gates.
