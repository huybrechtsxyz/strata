...
1. **`build_run()` starts.** If `--env-file` was passed (repeatable), each file is parsed and merged into the real `os.environ` via `setdefault` (Q9 — a real, already-exported env var always wins over a file's value). Then `build_path` is cleaned if requested.
2. **Immediately after, once, deployment-wide (not per-provisioner):** a new `build_controller.py`-level step resolves the deployment's reachable environments and calls `merge_environment_models()` to get `(variables, secrets, features)` typed dicts (Q1/Q4). From these it builds, purely locally, no network:
   ...
   - `properties`/`custom: dict[str, Any]` — deep-merged `workspace.spec.{source}` → `environment.spec.{source}`(+`overrides.properties`) → the deployment's own `spec.{source}` (Q3 — a deliberate improvement over v1's actual, docstring-contradicting behavior).
   - These five values populate new fields on `ResolvedWorkspaceGraph`, built once and handed unchanged to every provisioner (Q4).
...
flowchart TB
    ...
    subgraph ONETIME["Once, up front, deployment-wide - Q1/Q3/Q4/Q6"]
        ...
        SPECM["workspace to environment(+overrides.properties) to deployment's own<br/>spec.properties/custom deep-merge<br/>produces graph.properties / graph.custom"]
        GRAPH["ResolvedWorkspaceGraph now carries all five fields"]
        MANIFEST["write build_path/resolved.yaml - Q8<br/>near-direct dump of variable_refs+feature_refs+secret_refs<br/>value shown only where already safe - constant/environment"]
    end

    subgraph RESOLVEFLAG["Only when --resolve is passed - opt in, Q6/Q7"]
...
All 9 open questions now have a decision. Nothing left to design here — next is implementation:

1. Add `ValueReference` and the new `variable_refs`/`feature_refs`/`secret_refs`/`properties`/`custom` fields to `ResolvedWorkspaceGraph` (Q4).
2. Add the new `build_controller.py`-level function that computes all five once, before the per-provisioner loop, from `merge_environment_models()` (Q1/Q3/Q4) — this replaces the old `build_time_keys()`/`resolve_values()` call in the default path (Q6); delete `build_time_keys()`.
3. Implement Q3's workspace/environment(+overrides.properties)/deployment's-own merge chain for `properties`/`custom`.
4. Update `TerraformIntegration.default_output()` to read the new `graph` fields directly for the four new categories (Q1/Q2) — filtering `variable_refs`/`feature_refs` to `value is not None` entries.
5. Add the `--resolve` flag (Q7): when set, call `resolve_values()` over all declared keys (reusing the same dicts from step 2), report via `Diagnostics`, never write values to disk.
6. Add the `resolved.yaml` manifest (Q8), written once by `build_controller.py`, serializing `graph.variable_refs + graph.feature_refs + graph.secret_refs` (+ `properties`/`custom` if useful).
7. Add `--env-file` (Q9): a small inline `.env` parser (new util, e.g. `strata/utils/env_file.py`) plus the repeatable CLI option, applied via `os.environ.setdefault()` at the very start of `build_run()`.
8. Tests; full check suite; update `build-command.md`'s `default_output()` table + pipeline diagram + changelog; update ADR-0022 D1a's `build_time_keys()` cross-reference.
9. **Only after this is real**, design `OutputProfileModel`/`emits` gating on top of it (the original parity-gap ask) — a separate step, do not conflate.
