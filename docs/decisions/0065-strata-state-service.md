...
- **`GET /healthz` payload:** `{"status": "ok", "version": "<strata version>"}` — the version is small, genuinely useful diagnostic value (confirms which server build answered) without becoming aggregation or enrichment of anything ingest-related.
