...
- Status: partially-implemented — Phase 1 CLI/YAML foundation ✅ Done (including `--format svg|png` export via Kroki); Phase 2 workspace connection ✅ Done (URI scheme, `diagram resolve`, `click` directives, VS Code click-to-open-file, hover tooltips, reverse cursor→node lookup, `strata validate --deep` link-rot check); Phase 3 source coverage + cookbook ✅ Done (all built-in sources, all 10 Top-10 built-ins, all 5 non-flowchart template types, `strataDiagrams` sidebar view, `/diagram` chat command — see checklist for what was honestly scoped down); Phase 4 GUI Builder + AI chat ✅ Done (Builder's own SVG/PNG export buttons are a small remaining follow-up — the CLI capability exists, the Builder UI only wires up Mermaid-markdown export so far); Phase 5 advanced features not started
...
- Phase 1, 2, and 3 are functionally complete. Remaining Phase 2 item (`nodeMap` enrichment cache) is deferred, not blocking — only needed if secondary node actions beyond open-file are added later. Phase 3's cookbook/sidebar/chat items are done, including all 5 non-flowchart template types (`gantt`/`pie`/`sequence`/`quadrant`/`sankey`), though the sidebar itself is scoped down from its original description (no 185-entry catalog data exists to browse; generic `/diagram` command instead of 10 bespoke ones). Phase 4 (Visual Builder, round-trip, NL→diagram chat, Mermaid export) is done, and `strata diagram show --format svg|png` (Phase 1) now works via Kroki — no more blockers there. Still outstanding overall: per-data-source icon conventions, wiring SVG/PNG export buttons into the Builder's own UI (the CLI capability already exists, just not surfaced there yet). See "Implementation Roadmap" for the authoritative checklist.
...
- [x] Export to Mermaid markdown — "Copy Mermaid" button renders the current Builder state and copies the Mermaid source to the clipboard, matching the ADR's "no lock-in" principle (paste into a README/wiki/GitHub markdown). SVG/PNG export is now available at the CLI level (`strata diagram show --format svg|png`, via Kroki — see the Phase 1 checklist) but the Builder's own Export button still only wires up the Mermaid-markdown path; adding SVG/PNG buttons to the Builder UI is a small remaining follow-up, not a capability gap.
