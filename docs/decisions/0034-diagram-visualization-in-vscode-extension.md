...
- Status: partially-implemented — Phase 1 CLI/YAML foundation ✅ Done; Phase 2 workspace connection ✅ Done (URI scheme, `diagram resolve`, `click` directives, VS Code click-to-open-file, hover tooltips, reverse cursor→node lookup, `strata validate --deep` link-rot check); Phase 3 source coverage + cookbook ✅ Done (all built-in sources, all 10 Top-10 built-ins, `strataDiagrams` sidebar view, `/diagram` chat command — see checklist for what was honestly scoped down); Phase 4 GUI Builder + AI chat ✅ Done except SVG/PNG export (blocked on the still-unimplemented Phase 1 `--output svg|png` item); Phase 5 advanced features not started
...
- [x] Export to Mermaid markdown — "Copy Mermaid" button renders the current Builder state and copies the Mermaid source to the clipboard, matching the ADR's "no lock-in" principle (paste into a README/wiki/GitHub markdown). **Not done:** SVG/PNG export — blocked on the still-unimplemented Phase 1 item below (`strata diagram show --output svg|png`, needs mermaid-cli); revisit once that CLI capability exists.
