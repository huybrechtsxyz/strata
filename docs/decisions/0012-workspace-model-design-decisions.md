...
- `WorkspaceService._validate_dynamic()`: load each referenced `Topology`
  document and cross-check its `components[].resource`/`namespaces[].namespace`
  against this workspace's own `resources`/`namespaces` — the Phase 2 check
  ADR-0011 already anticipated. Same for each referenced `Network` document:
  cross-check `resource.subnet.subnet` against that network's real
  `NetworkDefinitionModel.subnets[]` (Decision 7).
