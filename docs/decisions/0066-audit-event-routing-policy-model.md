...
1. **Cloud provider identity** — resolved from whichever of `azure_cli` / `aws_cli` / `gcloud_cli` backs the workspace's configured provisioner(s) for the deployment being audited. Azure contributes the `user.name` from `az account show`; AWS contributes the `Arn` (or the IAM user/role segment parsed from it) from `get_identity()`; GCP contributes `get_account()`. If a workspace's provisioners span more than one provider, the identity is taken from the provider handling the stage the event belongs to, not resolved once globally.
2. **CI actor env vars** — `GITHUB_ACTOR`, then `BUILD_REQUESTEDFOR` (Azure DevOps) — used when no cloud integration is configured, or the configured one's identity call fails (not logged in, CLI unavailable). This is the same fallback chain `run_build_command.py` already uses today (`GITHUB_ACTOR or USER or USERNAME`), extended with the Azure DevOps variable so both CI platforms strata already targets are covered.
3. **OS login** (`$USER` / `os.getlogin()`) — the last resort, when neither a cloud identity nor a CI actor variable is available (local runs outside CI with no cloud CLI signed in).

This sharpens, rather than removes, the earlier caveat that **`actor` is self-asserted, not authenticated**: that caveat now applies precisely to steps 2 and 3 — `GITHUB_ACTOR` is a runner-supplied environment variable and `$USER` is trivially set, so treat those as claims, not proof. Step 1 is different in kind: a cloud CLI identity was authenticated *by the provider*, so it is externally verifiable against that provider's own audit trail (Azure AD sign-in logs, AWS CloudTrail, GCP Cloud Audit Logs) even though strata itself does not verify it further. ECS `user.name` carries whichever value resolved, and does not distinguish the two cases in the schema — that distinction is documentation, not a new field, to avoid the same kind of dead-configuration complexity this ADR exists to remove elsewhere.

This was the one decision in this section that could corrupt *stored* records if revisited later, and it no longer blocks implementation.

**Out of scope, on purpose:** this resolves `actor` for CLI-invoked commands and events only — no login, no server, no session. The moment a *user* (rather than a CI runner or a local CLI invocation) needs to authenticate to a *server* — ADR-0065's Phase 3 control plane, any future dashboard or approval UI — the problem changes shape: OIDC/OAuth2 login, session or token issuance and revocation, mapping an IdP identity onto strata's `actor`, and an authorization model for who may approve or deploy where. ADR-0065 already names that gap without resolving it (its open question 5, and its Phase 3 description of needing "an authorization model for who may deploy where"). [ADR-0067](0067-server-identity-authentication-authorization.md) settles that separately rather than improvising it inside either this one or ADR-0065 — it involves choices (IdP/protocol, session vs. token model, in-package vs. separate service) that neither ADR is scoped to make. Until Phase 3 is scheduled, ADR-0065's bearer tokens remain deliberately coarse (append-only, per-workspace, machine-to-machine) rather than a stand-in for user identity, and ADR-0067 settles that a control-plane session outranks this ADR's CLI-side resolution chain when one exists.
