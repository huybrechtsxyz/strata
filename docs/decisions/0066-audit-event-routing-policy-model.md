...
**Two more producers wired: `lock.acquired`/`lock.released` and `drift.detected`.** ✅ Done. Of the five event types with no producer (Consequences → Good: "the four unwired event types"), two had a clean, single, unambiguous call site with no layering conflict — wired in now rather than left as further plumbing debt:
  ...
  - **Two genuinely remain unwired, deliberately** (see the earlier "look at the events with no trigger" analysis): `policy.violated` and `secret.accessed` both have their producing logic in `validators/`/`utils/` respectively — layers that sit *below* `controllers/` in ADR-0003's chain, so `PolicyEngine`/`ResolvedValues` cannot call `AuditController.forward()` directly without a layering violation. Wiring these properly means adding the forward() call at each of their several command-layer call sites (`validate`, `build`, `deploy`, `check_policy` for policy; wherever `ResolvedValues` is invoked for secrets) via a shared helper — real, scoped work, not a quick patch, and left as an explicit follow-up rather than rushed. `deployment.measured` remains unwired because ADR-0064's metrics record itself is not yet implemented — there is no producer to wire.
