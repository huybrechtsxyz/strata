...
|-------|-------------|---------|
| `init` | — | Initialize workspace (creates `.platform/`) |
...
Always pass `--output json` (or set `XYZ_OUTPUT=json`). JSON responses use a standard envelope:
...
The CLI auto-discovers the workspace by walking up from CWD looking for a `.platform/` directory. Override with:

1. `--work-path /explicit/path` (highest priority)
2. `XYZ_WORK_PATH=/env/path` (environment variable)
3. Automatic upward walk from CWD

**Recommendation:** Set `XYZ_WORK_PATH` in your environment to avoid ambiguity.
...
| Variable | Content |
|----------|---------|
| `XYZ_PHASE` | Current lifecycle phase (e.g., `deploy_provision_before`) |
| `XYZ_WORKSPACE_PATH` | Path to workspace root |
| `XYZ_CONFIG_PATH` | Path to configuration files |
| `XYZ_BUILD_PATH` | Path to build artifacts |
| `XYZ_OBJECT_PATH` | Path to objects directory |
...
3. **Set `XYZ_WORK_PATH`** — eliminates ambiguity about which workspace you're targeting.
