...
- **CLI flow:** `xyz project init` → `xyz project add <repo>` → `xyz build` → `xyz deploy`
- **Workspace state:** `.xyz_platform/` folder in the workspace root
- **Work-path resolution:** `--work-path` flag > `XYZ_WORK_PATH` env var > walk up from CWD
