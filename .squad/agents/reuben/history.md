...
- **2026-07-28 — Hash-before-storage cookbook:** When a feature request is rejected in favor of a docs-only recipe, the recipe doc should explicitly restate *why* it's a recipe and not a feature (with the concrete risk that killed the code option) — readers who stumble on the doc later need that context without digging up the design-discussion log.
- **2026-08-03 — `.github/CHANGELOG.md` vs `.github/HISTORY.md` split (security fix entry):** Confirmed the two-file convention: `CHANGELOG.md` is terse (1-3 lines per user-facing capability, per repo memory `changelog-conventions.md` — point to detail elsewhere, don't restate sub-phases/method names), while `HISTORY.md` gets the bold-bullet-plus-nested-sub-bullet treatment with actual file paths and method names, matching the `[1.6.0]` entries exactly. Both files keep an always-present empty `## [Unreleased]` section right after the intro, above the newest dated release — new work goes there until a version/release decision is made, never as its own heading. When there's no ADR backing a change (a direct bug-report fix rather than an ADR-tracked feature), don't fabricate an ADR reference — just describe the fix plainly. `### Security` is a valid Keep a Changelog sub-heading alongside `### Added`/`### Fixed`/`### Changed`/`### Breaking Changes` and hadn't been used in this repo's changelog before now.
