...
- **Configurable tenant file path resolution** — Tenant location is no longer hardcoded to `tenants/{code}.yaml`. A configuration can now declare a custom path via `spec.paths` by marking a convention with `resolves: tenant`, enabling multi-tenant directory layouts like `customers/{code}/customer.yaml`. At most one convention may declare `resolves: tenant`; both validation and build phases use the declared pattern if present, otherwise fall back to the built-in default. Fully backward-compatible — no custom convention required to maintain existing behavior.
