...
- **`deployment.spec.environments[].overrides.resources[].configuration`/`custom` and `overrides.modules[].configuration` had the same shallow-merge bug, despite comments explicitly claiming "Deep merge"** — `DeploymentService.apply_environment_overrides()` applied resource- and module-level `configuration`/`custom` overrides with `dict.update()`, silently dropping nested sibling keys not repeated in the override, in every deploy that used a resource or module override with nested configuration. Fixed to use the same shared `deep_merge()`.

## [1.9.1] - 2026-09-02

### Changed

- **BREAKING: `configuration.spec.layering` and `spec.layerings` are removed and merged into `spec.paths`; `deployment.spec.layers` changes shape** — strata shipped three overlapping mechanisms for "where a deployment sits in a hierarchy" (`spec.layering`/`spec.layerings` for artifact paths, `spec.paths` for repository layout, `spec.layers` for the values), using two different matching engines that behaved differently despite looking identical. `spec.layerings.scope` matched with plain `fnmatch`, where `*` and `**` both cross `/` — so schemes written to target increasing depths (`deploy/*/*/**`, `deploy/*/*/*/**`, …) all silently matched the same shallow paths and the deepest-looking one won regardless of the file's real depth. There was also no link between the two representations: every layer value had to be hand-typed into `spec.layers`, duplicating what the file's own location already encoded, and the two were free to silently disagree. Now one mechanism: a `spec.paths` convention declaring `resolves: layers` owns the hierarchy (segment names, order, per-segment `pattern`/`default`) and uses the genuinely segment-aware matcher; `deployment.spec.layers` becomes `{follows, segments}`, where any segment value omitted from `segments` is derived from the deployment file's own path, then falls back to the segment's `default`, then is simply "not applicable" (the `required` flag is gone — different deployments in one family legitimately have different real depths). `validate:` rules now check each segment's **resolved** value rather than only the raw path capture, so explicitly-declared values and shallower deployments are no longer skipped. Ambiguous auto-detection (two conventions matching one file) is now a hard error naming both, instead of a silent first-match pick. No backward-compatible fallback: because every model uses `extra: forbid`, an unmigrated configuration fails immediately with an "unknown field" error naming the removed field. See [configuration.md](../docs/config/configuration.md#migration-from-layeringlayerings) for migration steps, [HISTORY.md](./HISTORY.md#clarify-layering-and-path-resolution-adr-0072) for implementation details, and [ADR 0072](../docs/decisions/0072-clarify-layering-vs-path-convention.md) for design rationale.

### Added

- **`configuration.spec.custom`, and `validate:` rules can now reach freeform dict data** — `spec.paths` `validate:` rules of the form `spec.<field>[*].<attr>` resolved the dotted path with `getattr()` only, which silently returns nothing the instant it hits a plain dict. Any rule pointing into the existing freeform `spec.configuration`/`spec.properties` fields therefore compiled, ran, and validated *nothing* — indistinguishable from a rule that was never written unless someone tested the negative case. Path resolution is now dict-aware at both the traversal and per-item extraction steps, and a new generic `spec.custom` escape hatch (same shape as `spec.configuration`/`spec.properties`) gives arbitrary lists a home that `validate:` rules can actually check against.
