...
- **And a second, larger one found while checking that command's exit-code contract**: `--strict-ai-review` on `build plan` **never failed**. `_run_ai_analysis()` appends to `self._errors` when risk clears the threshold (or the gate cannot run at all), and its docstring asserted this "adds to `self._errors` so the caller propagates a non-zero exit code" — but `_run_plan()` returned `True` unconditionally, and `handle_command_exit` only consults `has_validation_errors()`, which build commands do not define. The command printed "❌ Plan blocked" and exited **0**. `deploy run`'s equivalent gate returns `False` and blocks correctly, so this was an inconsistency, not a design choice — and a gate that does not gate is worse than no gate, because CI reports green on precisely the change someone asked to stop. `_run_plan()` now detects a gate-recorded error and returns `False`. Deliberately narrow: only the `--strict-ai-review` path changes, so a plain `build plan` whose terraform stages fail still exits 0 as before. Whether *that* should also fail is a separate question, with its own CI blast radius, and is left open rather than folded into a bug fix.
