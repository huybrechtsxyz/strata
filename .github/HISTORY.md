...
- **Symptom**: a workspace with two Terraform provisioners — one consuming the shared environment fully, one an externally-authored root consuming two keys of it — failed the build with 21 × `Input 'x' is not declared in variables.tf`. There was no configuration that fixed it: ADR-0078's scoping computes `injected(P)` as a *union* of component requirements, and a union cannot narrow below itself, so the second root was always offered every environment key.
- **Root cause**: `check_inputs()`'s rule 1 treated every injected-but-undeclared key as an error, regardless of whether anything had actually asked for it. For an unscoped workspace, "injected" means "every key in the environment", so any key a root does not consume was fatal.
- **The failure was self-inflicted.** Terraform tolerates this: it emits `Value for undeclared variable` and plans normally. Verified by the integration team against the real root on Terraform 1.12.2 with all 21 keys — 2 shown, 19 aggregated, every one a warning, `Plan: 4 to add`. The only errors they could provoke were a *declared* variable left blank and a genuine data-source lookup failure. So the check, not Terraform, was blocking the build.
- **Fix**: `check_inputs()` gained a keyword-only `referenced_keys`, splitting rule 1 by **who asked**. A key named by a component's `spec.references`, by a provisioner's, **or by a matching stage's `secrets:` allowlist**, that the root cannot accept, stays an **error** — something requested it and the request cannot be satisfied. A key merely present in the environment becomes a **warning**. Omitting the parameter preserves the original all-errors behaviour, so no pre-existing call site moves.
- **The "did you mean?" suggestion is attached to both severities**, at the integration team's request and for good reason: two of their 21 carried real signal (`daily_quota_gb` → `log_workspace_daily_quota_gb`, `web_apps_subnet_name` → `vnet_default_subnet_name`) — genuine naming mismatches between a shared environment and an externally-authored root, which is precisely what needs reconciling when wiring an estate. Demoting the severity must not demote the content; a comment in the code says so.
- **An existing test caught a hole in the first implementation.** `test_genuine_typo_still_reported_as_error` uses `stages[].secrets: ["TYPO_SECRET"]` — an explicit request made through the stage allowlist rather than `references`, which the initial `referenced_keys` computation missed and wrongly demoted. `_compute_referenced_keys()` now folds in **literal** stage-secret names; `['*']` and the no-matching-stages fallback are deliberately excluded, since neither names a particular key. That distinction is now pinned by its own test.
- **Scope, deliberately narrow**: this changes *severity*, not delivery. The external root still receives all 22 variables and 7 feature flags — Terraform simply tolerates them. It is not per-provisioner scoping and is not a substitute for it; see [ADR-0084](../docs/decisions/0084-variables-secrets-features-delivery-model.md), which records the model question this defers.
- **Behaviour change worth knowing**: an unscoped workspace that previously failed the build on an unused environment key now succeeds with warnings.
...
- **Severity**: same class as `deploy show --stage` (documented, never wired), but on a deployment safety gate rather than a preview. Anyone who had adopted `--strict-ai-review` in a pipeline believed their applies were risk-gated. Click rejects unknown flags, so the failure was at least loud at invocation rather than silently permissive.
- **Fix**: declared both options on `deploy run`, positioned to match the existing parameter order. No logic changed — the gate worked as written.
- **Tests**: new `tests/strata/commands/test_commands_deploy_ai_gate.py`, 28 tests. The gate had **zero** coverage, so enabling it untested would only have moved the problem: from "cannot block" to "can block wrongly". Covers the strict/advisory split, `--force` overriding advisory mode but **not** strict mode, TTY vs CI behaviour, provider-missing and provider-error paths, and `_parse_ai_risk`'s JSON and prose fallbacks. Every test passed first time — the logic was sound, only unreachable.
- **Exit code — 1, not 3.** `build plan --strict-ai-review` had just been given exit `3`, so matching it looked obvious. Checking first showed it would have been wrong: every existing gate denial in `deploy run` — policy deny, approval gates, post-plan condition gates — already exits `1`, and the command's documented epilog reserves `3` for schema/cross-ref validation. Consistency *within* the command beat consistency with a different command carrying a different documented contract. The draft help text promising "exit 3" was corrected before it shipped.
  - Recorded as a known deficiency rather than a decision: on `deploy run`, "a gate deliberately said no" and "the infrastructure broke" are both exit `1`, so a pipeline that retries on `1` will retry a deliberate block. Fixing that means reassigning an exit code across *every* `deploy run` gate, which needs its own ADR.
- **Placement verified, not assumed**: the gate sits inside `if step_name == STEP_PLAN and STEP_APPLY in steps_to_run:`, and `--dry-run` builds a step list without `STEP_APPLY` — so a preview neither burns an AI call nor can be blocked by one.

#### **`deploy show` made a faithful preview of `deploy run`**

- **Problem, two of them.** (1) ADR-0083 introduced stage gating but left `deploy show` — the preview surface — unable to disclose it: a user read `enable_api: false` in the features section and `api` in the stage list and joined them by hand, which is the confusion ADR-0083 removed from the audit trail, relocated to the preview. (2) Independently and predating it, `deploy show --stage X` was **documented as working and never implemented** — `self._stage` was stored and never read, so the flag was silently ignored.
- **Fix (1) — disclose, never filter.** Stage rows gained `enabled`, `would_skip` and `skip_reason`, via the same `evaluate_enabled()` call `build plan` already makes, so the two previews cannot disagree. Stage selection runs in `StageSelectionMode.INSPECT`, which is load-bearing: `DEPLOY` would gate disabled stages out and leave the new marker with nothing to mark. Per ADR-0083 D11, a read-only surface must never *filter* on `enabled` — filtering would make "disabled in this environment" indistinguishable from "deleted from the deployment".
- **Fix (2) — give `--stage` the meaning it has everywhere else.** [ADR-0020](../docs/decisions/0020-cli-parameter-consistency-standard.md) never listed `--stage` for `deploy show` at all, but does specify it for the analogous `deploy plan` ("Limit display to specific deployment stage"); the flag had been added outside the standard. Unifying it is a *return* to the standard, not a deviation — and since it was never wired up, nothing that worked before broke. `--scope` was added alongside, for parity with `run`/`destroy`.
- **Secrets scoping.** With `--stage`/`--scope`, the secrets view narrows via the shared `allowed_secret_keys_for_stages()` to what those stages would actually receive. Variables and features are deliberately *not* narrowed, mirroring the deploy-time contract in `ResolvedValues.for_stage()` where only secrets carry a per-stage allowlist.
- **Out-of-scope secrets are listed, not omitted.** Omission was simpler and was rejected: a vanished secret is indistinguishable from one that was never declared — ADR-0083's "excluded vs. absent" problem one level down — and *"why can't my stage see `SECRET_FOO`?"* is precisely the question omission answers worst. The row carries `in_scope: false` with a null value. Withholding the value is modelling, not a security boundary: the same operator sees it by dropping `--stage`.
- **Scoping is tied to the flag, not to stage selection.** The design assumed that with no flags, the union of every stage's allowlist reproduces today's unscoped view. Checking `ResolvedValues.for_stage()` disproved it: a stage with no `secrets:` receives **no** secrets, so for any deployment that declares no allowlists at all the union is *empty* — every secret would have silently become `null` in the default view. Scoping now applies only when the operator actually narrowed the selection.
- **An unresolvable `enabled` expression is reported**, not rendered as a stage that looks fine. `evaluate_enabled()` returns an error (not a skip) for that case; `deploy run` would abort on it, so a preview that stayed silent would be inaccurate in exactly the way this work exists to prevent.
- **Supporting refactor**: `_resolve_values(strict=False)` was hoisted out of `_collect_environment()` into `_collect()` and threaded down. Both the stage rows and the environment detail need it, and its ADR-0026 cache covers variables and features but **never secrets** — so a second call would re-read every secret store. A test asserts it is called exactly once.
- **Tests**: new `tests/strata/commands/test_commands_deploy_show.py` — the command had shipped with no test module at all, which is how a documented-but-absent flag survived. 34 tests: characterisation of the pre-existing output shape (so the hoist is provably inert), gating disclosure, selection, secret scoping, and CLI flag wiring.
- **Docs**: `commands.md` and ADR-0020 corrected; the ADR carries a note recording that the flag was added outside the standard and later brought back into it.
- **Follow-on fix in `build plan`**: the same inaccuracy existed one command over. `_plan_stage()` called `evaluate_enabled()` and discarded its error return (`_enabled, skip, _error = ...`), so a stage whose `enabled` expression could not be resolved was planned and rendered as ordinary and unmarked — the worse half of the silence the `would_skip` marker exists to break, since a disabled-but-unmarked stage at least resolves, while this one is a run that would abort with nothing on screen to say so. The error is now surfaced as the row's `error` and the stage returns early: planning a stage that cannot run produces nothing actionable.
- **And a second, larger one found while checking that command's exit-code contract**: `--strict-ai-review` on `build plan` **never failed**. `_run_ai_analysis()` appends to `self._errors` when risk clears the threshold (or the gate cannot run at all), and its docstring asserted this "adds to `self._errors` so the caller propagates a non-zero exit code" — but `_run_plan()` returned `True` unconditionally, and `handle_command_exit` only consults `has_validation_errors()`, which build commands did not define. The command printed "❌ Plan blocked" and exited **0**.
- **Which opened a third question: what else was `build plan` swallowing?** Three outcomes all exited `0`, and separating them was most of the work:
  - *plan ran, changes found* — **correctly** exit `0`, and must stay that way. The shipped `build-plan` GitHub Action surfaces changes as a `has_changes` **output** and its documented recipe branches on it; an exit code here would break every PR-preview pipeline. ADR-0004 claimed `build plan` used exit `3` "when plan shows changes in strict mode" — a mode that never existed (there is no `--strict` flag), now corrected in the ADR.
  - *plan could not run* — a stage whose terraform init/validate/plan failed set `error` on its row and was still reported as success. Actively misleading, not merely quiet: the Action's `has_changes` jq is `.data.terraform_plan | length > 0`, i.e. row **presence**, not row success — so a wholly broken plan was announced to CI as "plan shows changes". Now exit `1`.
  - *gate rejected a working plan* — now exit `3`, matching the flag's own help text ("Fail (exit 3)") and ADR-0004's "fix the config, block the PR" semantics.
- **The split is the point.** A pipeline may retry exit `1` (transient infrastructure) but must never retry exit `3` (a human rejected the change). Collapsing both to "non-zero" would have been simpler and would have lost exactly the distinction CI needs. When both occur, exit `1` wins: if terraform never produced a plan, the AI reviewed nothing meaningful.
- `_gate_blocked`/`_plan_failed` are **class** attributes rather than `__init__` assignments, because `_run_plan` reads both unconditionally and the command is routinely constructed via `__new__` in tests — a missing attribute would turn a reporting bug into an `AttributeError` crash. Same reasoning as `StageSelectionMixin`'s declared attributes.
- The `build-plan` action and its README were updated to distinguish a blocked plan from a broken one in the step summary, and to document that `has_changes` is only meaningful on exit `0`.

#### **Declarative stage gating — `stages[].enabled` (ADR-0083, Phases 1–8)**

- **Problem**: there was no declarative way to say "this stage does not apply in this environment". The only levers were `--stage`/`--scope` (imperative, per-invocation) or maintaining duplicate deployment files per estate. The consequence was an audit-trail gap, not just an ergonomic one: a stage that didn't apply was simply **absent** from the manifest and deploy-log, indistinguishable from "the deployment never mentioned this stage" or "deployed, no changes".
- **Field**: `stages[].enabled` accepts a literal boolean or an existing `${feature:KEY}`/`${var:KEY}` expression (ADR-0075's shared resolver — no new resolution machinery, one more call site). Omitted means "always run", so every pre-existing deployment file is unaffected. `enabled` was chosen over `condition`/`when`/`if` because it is already strata's de-facto inclusion keyword in six live schema positions; `if` is additionally unusable as a Python attribute name. No alias is accepted — a synonym hint in the validator's "did you mean" path is the planned ergonomic answer instead.
- **Gating applies to changes, never to observation**: `deploy run` is gated; `deploy destroy`, `build run`, `build plan` and drift detection are not. Gating destroy would strand infrastructure created before a flag was turned off — the orphaned-resource failure mode is precisely what drift detection against a disabled stage is for. `enabled: false` therefore means "stop deploying this", **not** "destroy this", and is documented as such.
- **Evaluated before pre-flight**, not inside the stage loop: `_preflight_check_provisioners()` constructs a deployer and runs `validate_workspace()`/`validate_environment()` per stage, so gating any later would still require a disabled estate's tooling and cloud auth to be present. The test for this asserts `_create_deployer` is **not called**, rather than merely that the stage didn't deploy — the latter would pass even if a backend had already been initialised.
- **`--stage` cannot select a disabled stage** (hard error). This deliberately departs from the `helm_namespaces`/`--namespace` precedent: that field is a *scoping* list, whereas `enabled` is a *correctness condition* for the environment, and forcing it would create infrastructure the environment declares inapplicable. Relaxable later behind an explicit flag; the reverse is not.
- **Skips are recorded, never silently absent**: `ManifestStageModel.status` finally emits its long-documented `"skipped"` value, with a new `skip_reason` naming both the expression and what it resolved to. `DeployLogStageModel` gained `status` + `skip_reason` (`success` unchanged for compatibility), with `status` derived from `success` when not supplied so existing constructions keep working. Skips are recorded immediately after selection rather than after the stage loop, so the fact survives an abort later in the same invocation.
- **Validation**: `${secret:KEY}` in an `enabled` gate is rejected outright — the skip reason is persisted to the manifest, so gating on a secret would copy it into an audit artifact. An undeclared `${var:}`/`${feature:}` key fails `strata validate`. The declared-key lookup falls back to raw YAML parsing (the technique `_validate_helm_stage_namespaces` already uses) so it works during a plain validate, and returns "unknown" rather than a partial set when any environment file can't be resolved — a partial set would produce false "not declared" errors.
- **Two pre-existing duplications fixed rather than extended** (ADR-0073's "one implementation, not copies"): the `--stage`/`--scope` filter was copy-pasted between `run_deploy_command` and `destroy_deploy_command` and had already drifted to *different* `--stage` not-found messages — now one shared `strata/utils/stage_selection.py`. The string→bool truthiness rule appeared twice inside `base_builder.py` — now one shared `parse_bool()` in `resolved_values.py`, with parity tests pinning the extracted behaviour.
- **Cascade semantics deliberately scoped**: `enabled` skips propagate transitively through `depends_on` (GitHub Actions' `needs:` default), with each reason naming the *direct* dependency so a chain can be walked back one hop at a time. Separately and permanently, `--stage`/`--scope` narrowing never cascades — cascading it would make `--stage B` impossible whenever `B` declares a dependency.
- **`depends_on` gained runtime meaning for the first time** (Phase 5). It previously only drew diagram edges, so a typo'd or circular graph was completely inert. Execution now uses Kahn's algorithm with ready nodes dequeued in **declaration order** — the tie-break is the entire safety story, because it guarantees a file already in a valid order comes back unchanged. Verified against every shipped example deployment (5 files, all unchanged) in addition to unit tests. Files declared *out* of dependency order will now be reordered, and dangling/self/cyclic references now fail validation — both recorded as behaviour changes in CHANGELOG. `deploy destroy` deliberately keeps declaration order: a correct teardown needs the *reverse* order, which is a separate decision this ADR does not make, so destroy takes an explicit `apply_ordering=False` rather than silently inheriting the wrong one.
- **One implementation for ordering and validation**: `DeploymentService._validate_stage_depends_on()` delegates to `stage_selection.validate_stage_dependencies()`, which itself reuses `order_stages()` for cycle detection — so validation and execution cannot disagree about what a valid graph is. Dangling references are reported *without* the derived cycle complaint they would otherwise trigger, so a concrete typo is not buried under a consequence of itself.
- **Testing**: ~105 new tests across `test_utils_stage_selection.py`, `test_utils_resolved_values.py`, `test_stage_enabled.py`, plus command-level gating/recording/ordering classes. Full suite 6932 passed, 16 skipped; ruff/mypy/Sphinx green. A latent test hazard was fixed along the way: the shared `_make_stage()` helper built bare `MagicMock`s without `enabled`/`depends_on`, and an unset `MagicMock` attribute is truthy *and* iterates as empty — so gating tests would have silently exercised the wrong branch.
- **Phase 7 — cross-schema cleanup**: a full sweep of `src/strata/models/` for `enabled`/`condition`/`when` found 15 positions; 6 were live, 2 marginal, 4 inert, 2 belong to different concepts. `resources[].condition` and its environment override are **removed** — inert *and* redundant, since an environment can already override `enabled`; its documented `'{{ environment }} == production'` syntax was a fifth expression shape that no engine had ever parsed, and the `sln init` scaffold advertised it to every new user. `resources[].enabled` and `resources[].modules[].enabled` are now **implemented**: a new `collect_disabled_names()` in `platform_builder.py` computes the excluded set once, so a disabled resource/module never enters the platform artifact and therefore reaches no provisioner — one choke point instead of a filter in each of the terraform/ansible/platform builders. Module keys are `"<resource>:<module>"`, matching `WorkspaceService.get_module_services()`, so the same module can be enabled on one resource and disabled on another. Deliberately a *build* concern, not a *loading* one: a disabled resource's YAML is still loaded and validated, so mistakes surface before someone switches it back on. That the entire 6947-test suite passed unchanged after the removal is itself the evidence of how inert these fields were. `enabled` now means the same thing on all 11 positions that carry it. Full suite 6965 passed, 16 skipped.
- **Phase 8 — `StageSelectionMixin`**: the `--stage` filter turned out to have **seven** copies, not the two D7 originally recorded, with three different messages between them — `deploy run`/`destroy` plus `deploy status`/`plan`/`health`/`drift` and `build plan`. All seven now share `StageSelectionMixin._resolve_stages()`, inherited by both `BaseDeployCommand` and `BaseBuildCommand`. The mixin absorbs four duplicated blocks, not one: the null-service guard (itself a second duplication, with `status` carrying its own "Deployment model not loaded" variant), the `spec.stages` extraction, the filter, and the error routing. `select_stages`' `apply_gating`/`apply_ordering` booleans — four combinations, two meaningful, wrong pairings failing silently — were replaced by a `StageSelectionMode` enum (`DEPLOY`/`DESTROY`/`INSPECT`) living in `utils/` on `select_stages` itself, so invalid states are unrepresentable for every caller including tests. `DESTROY` and `INSPECT` behave identically today but are kept distinct because their *reasons* differ (orphan-safety vs. read-only). **A `StageController` was designed first and rejected on review**: its body would have been three lines of delegation, the mixin had to exist anyway for two-line call sites, and the testability argument for it proved false — `test_commands_stage_mixin.py` needs only a five-line stub class, easier than a controller test would have been. **`output_deploy_command` is deliberately not migrated**: it filters by stage *type* before `--stage` and its error covers "exists but is the wrong kind", a case the shared helper has no concept of — routed through it, a named non-terraform stage would be returned, the type filter would empty the list, and the command would report nothing at all. Full suite 6981 passed, 16 skipped.
- **Phase 6 polish**: (a) `test_commands_gating_not_applied.py` pins the non-gated surfaces — `build plan` and drift detection must keep seeing a disabled stage, because a future refactor routing them through `select_stages` with gating on would regress silently and the failure mode (invisible orphaned resources) surfaces only in a bill or an audit; (b) `build plan` now sets `would_skip`/`skip_reason` on a disabled stage's plan result, so a preview distinguishes "disabled" from "deleted"; (c) `strata validate` gained a `_FIELD_SYNONYMS` map (`condition`/`when`/`if`/`disabled` → `enabled`) consulted before the existing `difflib` pass — verified necessary, since `difflib.get_close_matches("condition", ["enabled"], cutoff=0.5)` returns nothing. The hint only fires where `enabled` is genuinely a valid field for that schema position. Full suite 6947 passed, 16 skipped.

### Fixed

#### **`CheckovPolicy` silently passed a `deny`-enforcement security policy without scanning anything**

- **Symptom**: a `type: checkov`, `phase: build`, `enforcement: deny` policy always reported `passed=True` for any workspace whose terraform provisioner used a nested `source.source_path` (e.g. `control/terraform`) — the common case for any multi-provisioner workspace. The build stayed green and the policy appeared enforced, but nothing was ever scanned.
- **Root cause**: `CheckovPolicy._resolve_terraform_dir()` hand-rolled its own three-candidate artifact-directory guess (`{build_path}/{deployment_name}/terraform/`, `{build_path}/terraform/`, `{build_path}/` itself), none of which consulted a provisioner's `source.source_path`/`target_path`. `SolutionController.get_provisioner_path()` — the single source of truth `TerraformBuilder` (copy destination) and `TerraformDeployer` (working directory) already share (ADR-0071) — was never adopted by this policy. Compounding this, `run_build_command.py`'s `_evaluate_build_policies()` (the code path `strata build run` actually uses) never passed `deployment_service` into `PolicyContext` at all, so even the policy's own best-case candidate was dead code in production.
- **Fix**: `CheckovPolicy` now resolves each scanned provisioner's directory via `get_provisioner_path()` (or the same `target_path or source_path` fallback shape `TerraformDeployer` uses, for library/test use without a `SolutionController`). A new `configuration.scope` field (`staged` default | `all` | `<stage-name>`) controls which terraform provisioner(s) are scanned when a workspace declares more than one (e.g. a root `control_infra` provisioner plus a shared `core_modules` module-library provisioner that no stage targets directly) — `staged` scans only provisioners reachable from at least one deployment stage. When multiple provisioners are scanned, any single breach denies the whole policy result, with findings still reported per provisioner in `PolicyResult.details["provisioners"]` and violations prefixed with the provisioner name.
- **Also fixed — silent skips made visible**: `PolicyResult` gained a `warnings: List[str]` field; every one of `CheckovPolicy`'s graceful-degradation paths (no artifacts found, Checkov not installed, scan subprocess failed, invalid `severity_gate`/`scope`) now populates an explicit, actionable message, surfaced as a `⚠` line by every `PolicyContext` call site (`strata build run`, `strata policies check`, `strata deploy run`) even when the policy technically passed.
- **Related refactor — one provisioner-resolution implementation, not three**: the "which deployment stage targets which provisioner" logic existed independently in `BaseDeployer._resolve_iac_model()` and `TerraformBuilder._stages_for_provisioner()` — and the two were not actually equivalent (`BaseDeployer` silently fell through to a later priority after a typo'd `stage.provisioner`; `TerraformBuilder` did not). Extracted into one shared `strata/utils/provisioner_resolution.py` helper used by both plus the new `CheckovPolicy` scope filter, standardized on the stricter (no silent fallback) behavior everywhere. **Compatibility note**: a workspace with a typo'd `stage.provisioner` that today silently deploys via a fallback provisioner (with only a buried warning log) will now fail `validate_workspace()` outright with the existing "cannot resolve a terraform provisioner" error instead.
- **Testing**: `test_checkov_policy.py` rewritten for the new multi-provisioner/scope-aware resolution (38 tests); `test_deployers_terraform.py`'s `_resolve_iac_model()` fall-through test updated to assert the new strict-failure behavior. Full suite (6707 tests) + lint/format/mypy/docs/smoke checks all green.
- **See**: ADR-0051's 2026-09-16 revision for the full design writeup.

#### **`CheckovPolicy` extended to Bicep and Ansible provisioners**

- Generalized the Terraform-only path resolution above into `_FRAMEWORK_PROVISIONER_MAP` (`terraform`/`bicep`/`ansible`, each mapped to its provisioner type and file glob) — `configuration.framework` now also selects the provisioner type via reverse lookup. `_resolve_terraform_dir()` renamed `_resolve_provisioner_dirs()`; `scope`/aggregation/`warnings` needed no changes since none of it was actually Terraform-specific.
- `helm`/`compose`/`argocd`/`flux`/`script` remain unsupported — an unrecognized `framework` skips cleanly with a message listing what's actually supported, instead of crashing.
- **Testing**: 12 new/updated tests (Bicep/Ansible end-to-end, `.yaml` glob variant, unsupported-framework skip). Full suite + lint/format/mypy/docs green.
- **See**: ADR-0051's 2026-09-16 multi-provisioner follow-up section.

#### **Helm's build-time secret-reference check was never scoped to the deploying stage**

- **Symptom**: a Helm module referencing `${secret:KEY}` where `KEY` is registered in the environment file but excluded from the deploying stage's `secrets:` allowlist passed `strata build run` cleanly — the failure only surfaced at real deploy time, with an error indistinguishable from "the secret was never registered at all". Cost a full deploy cycle to diagnose which of the two problems it actually was.
- **Root cause**: `HelmBuilder._validate_expr_refs()`'s own docstring already acknowledged this — unlike `TerraformBuilder`'s equivalent check (properly scoped to `stage.secrets` via `_stages_for_provisioner()`/`_allowed_secret_keys_for_stages()`), Helm's check validated against every secret declared anywhere in the environment, because no stage-to-namespace resolver existed for the helm provisioner at build time.
- **Fix**: found that `DeploymentStageModel.helm_namespaces` already provides exactly this resolver — a direct, single-hop "which namespaces does this stage deploy" mapping (default: every namespace, when unset), no topology traversal needed. New shared helpers in `strata/utils/provisioner_resolution.py` — `helm_namespaces_for_stage()`, `stages_for_helm_namespace()` — plus `allowed_secret_keys_for_stages()` (extracted out of `TerraformBuilder`, now shared) wire this into `HelmBuilder._validate_expr_refs()`. Two distinct error messages now exist: "not declared as a variable, secret, or feature" (never registered) vs. "declared in the environment but not included in the deploying stage's secrets: allowlist" (the new check) — same error class, immediately distinguishable. Variables/features remain unscoped (never stage-scoped at deploy time either). Non-breaking: no stages, or no stage matching the namespace, falls back to the legacy unscoped behavior.
- **Note**: `strata validate --deep` still doesn't perform this cross-check — this fix is build-time only (`strata build run`), which is an accepted scope boundary (build failures surfacing later in the pipeline than validate is normal; "build = compile").
- **Testing**: 5 new/updated tests in `test_builders_helm.py` (allowlist-excluded distinct error, allowlist-included passes, unmatched-namespace fallback, workspace-unavailable fallback). Full suite + lint/format/mypy/docs green.
- **See**: ADR-0051's "Related fix (2026-09-17)" section.

#### **`CheckovPolicy` extended to Helm — the previously "deferred" gap closed**

- Follow-up to the Bicep/Ansible generalization above and to the same-day discovery of `stage.helm_namespaces` (see the secret-scoping fix immediately above): the two blockers that made Helm-Checkov support genuinely hard — no single directory per provisioner, and no stage-to-namespace resolver — are now resolved or non-issues. `helm_namespaces_for_stage()` (already built for the secret-scoping fix) gives the namespace set directly; `deployment_service.get_namespace_services()` returns the same already-loaded `NamespaceService` objects `HelmBuilder` itself uses, so no re-parsing is needed.
- New `CheckovPolicy._resolve_helm_module_dirs()`: resolves target namespaces from `scope` (`staged`/`all`/`<stage-name>`), enumerates each namespace's modules, loads each via `ModuleService.load()` to filter `spec.type == helm`, and confirms each candidate is a genuinely scannable **local** chart via a copied `Chart.yaml` at `get_module_build_path()`. Registry-pulled charts (`source.chart_repository` set) have no local chart source at build time — skipped with an explicit warning ("no local chart source to scan"), never silently, since only `values.yaml`/`meta.yaml` exist for those. Findings are reported per `namespace/module` label, reusing the exact same aggregation/violation-prefixing/`warnings` machinery the other frameworks already use — no changes needed there.
- The `helm` CLI itself being unavailable (Checkov shells out to `helm template` to render before scanning) needed no special-case handling — a missing binary already surfaces as a scan failure through the existing graceful-degradation path.
- **Testing**: 13 new tests (local chart found, registry-chart skip+warning, mixed local/registry, missing `Chart.yaml`, non-helm module excluded, `helm_namespaces` allowlist scoping, invalid scope, `all` scope, `solution_controller` path, 2 end-to-end `evaluate()` tests). Found and fixed a real test-authoring bug along the way: `unittest.mock.MagicMock` auto-configures `__iter__` to return an empty iterator rather than raising, so a shared test helper that didn't explicitly set `.helm_namespaces = None` silently exercised the wrong code path (an unset Optional[list] mock attribute reads as "empty list", not "None") — recorded as a general gotcha for future test-writing.
- **See**: ADR-0051's Helm section — support implemented, no longer deferred.
...
- **Symptom**: every `strata build run` emitted `⚠ [control_infra] Required variable 'environment_info' (no default) is not supplied by any input`, for a variable that was in fact supplied and that Terraform consumed without complaint — the same build went on to run `terraform plan` successfully and produce a tfplan, which could not have happened had the variable genuinely been missing. The warning was unconditional and unfixable from the workspace side: no YAML change could silence it short of deleting the input.
- **Root cause**: `TerraformBuilder._validate_inputs()` compares the required variables declared in `variables.tf` against `declared_keys`, which `_collect_declared_input_keys()` builds from `spec.variables`, `spec.features` and stage-scoped `spec.secrets` **only**. `spec.properties` and `spec.custom` are written to their own tfvars files (`properties.auto.tfvars.json`, `custom.auto.tfvars.json`), so their top-level keys reach Terraform as real variable values — but they appear in neither `declared_keys` nor the `excluded` set (which covers only the 11 structural `STRATA_INJECTED_KEYS` and platform-emitted resource roles). Any workspace supplying a required input through a properties block therefore tripped the warning by construction.
- **Fix**: `_validate_inputs()` now adds the top-level keys of the merged `properties` and `custom` blocks to `excluded`. Two deliberate design choices:
  - **`excluded`, not `declared_keys`.** Adding them to `declared_keys` would also subject them to the *undeclared-variable* check (which is an error, not a warning), and a properties block shared across several Terraform roots legitimately carries keys that not every root declares — that would have turned a false warning into a false build failure. Extending `excluded` suppresses both checks and is strictly a relaxation, so it cannot break a build that passes today.
  - **The provisioner's `output:` profile is honoured.** The keys are only excluded when `OutputProfileModel.should_emit("properties"/"custom")` is true, so a provisioner using `format: none` — or an `emits:` list that omits the category — still gets the warning, because in that case the file genuinely isn't written and the variable genuinely isn't supplied. An absent profile (`None`) means the documented `format: strata` default, so everything is emitted and everything is excluded.
- **Testing**: new `TestValidateInputsPropertiesSuppliedKeys` class (4 tests) in `test_builders_terraform.py`. Two assert the warning is gone for a required variable supplied via `properties` and via `custom`; two guard against over-correction — a variable nothing supplies must still warn, and a variable supplied via `properties` must still warn when the profile's `emits:` omits `properties`. Verified the suite is meaningful by stashing the source fix and re-running: the two bug-catching tests fail without it while the two safety tests pass either way.
- **Note for ADR-0078**: this is the same underlying defect as the one that ADR is about, from the opposite direction. `declared_keys` is an inaccurate model of what actually reaches Terraform — ADR-0078's case is that it is too *broad* (a secret scoped to another provisioner's stage produces a false error), this case is that it is too *narrow* (a real input isn't counted at all). Both argue for deriving the check from what is actually wired rather than bolting on another declaration.
...
#### **`@repo/...` resolution depended on the process working directory, ignoring `--work-path`**

- **Root cause**: `SolutionController.get_repo_map()` used two different base paths for the two repository types. `type: gitops` repos resolved `r.path` against `self._work_path` (correct), but `type: local` repos resolved `r.url` against `Path(os.getcwd())` — reaching past `self._work_path`, which is in scope and used on the very next line. For the extremely common self-reference entry (`{name: "config", url: ".", path: "config", type: "local"}`), `repo_map["config"]` therefore became *the directory the user happened to be standing in*, not the workspace root.
- **Impact**: a live, reproducible failure, not a latent tidiness issue. With identical arguments — same explicit `--work-path`, same work-path-relative `-f` — a command run from the workspace root succeeded while the same command run from a nested deployment directory failed. A profile referencing `@config/config/base.yaml` resolved to `<work_path>/deploy/control/dev/config/base.yaml` instead of `<work_path>/config/base.yaml`. Two aggravating factors: **an explicitly-supplied, correct `--work-path` was silently ignored** (the user gave strata the right answer and it used CWD anyway), and **the diagnostic named the wrong thing** — it reported the profile's config references as the problem when those references were correct. Net effect: running `strata` from a subdirectory was unsupported in any workspace combining `type: local` repos with `@ref` references, and this was documented, warned, and validated nowhere.
- **Fix**: `get_repo_map()` now resolves a single normalized base (`Path(self._work_path).resolve()`) once and uses it for **both** repository types. Registered repositories are workspace-scoped state — they live in `.strata/solution.json` inside the workspace — so the workspace root is the only stable base. Both branches now also `.resolve()` their result, so the returned map is consistently absolute and normalized regardless of type.
- **Second instance of the same bug, fixed identically**: `SolutionController.generate_workspace()` resolved `type: local` repo urls against `os.getcwd()` for exactly the same reason, making the generated `.code-workspace` file's folder paths depend on which directory `strata repo add` was invoked from. Now uses the same workspace-root base.
- **Testing**: new `TestSolutionControllerRepoMap` class (5 tests) in `test_controllers_solution.py`. The headline regression test (`test_local_repo_resolves_against_work_path_not_cwd`) `monkeypatch.chdir()`s into a nested directory and asserts the resulting map is byte-identical to the one produced from the workspace root — which is precisely what failed before. Companion tests cover workspace-relative urls (`../sibling`), absolute urls being passed through unchanged, gitops CWD-independence, and `test_local_and_gitops_share_the_same_base`, which pins the actual root cause by asserting that a `local` and a `gitops` repo declaring the same relative path resolve to the same location.
- **Context**: this is Track 1 / item B-5 of ADR-0077. It is non-breaking, independent of every schema decision in that ADR, and stands on its own merits.
