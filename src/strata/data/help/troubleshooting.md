...
**Cause:** xyz cannot find a `.strata/solution.json` by walking up from the
...
If `xyz init` failed partway through, `.strata/` may exist in a partial state.

**Fix:**
```
xyz clean                          # removes .strata/ cleanly
