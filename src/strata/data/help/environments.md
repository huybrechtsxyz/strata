...
strata ref envfile add --profile prd --name base --path ./envs/prd.env

strata ref configfile add --profile dev --name app --path ./config/app-dev.yaml
strata ref configfile add --profile stg --name app --path ./config/app-stg.yaml
strata ref configfile add --profile prd --name app --path ./config/app-prd.yaml
```
...
```
strata profile activate stg    # switch to staging context
strata log list                # review what loaded
```
...
strata log list --level debug    # see which files loaded and what was skipped
cat .strata/configuration.yaml    # review merged config
```

Correct any missing refs or wrong paths before running build or deploy.

See also: `strata help --topic profiles`, `strata help --topic config-merge`
