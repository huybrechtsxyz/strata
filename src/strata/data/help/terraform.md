...
- Useful environment variables and parameters:
  ...
  - `TF_CLI_CONFIG_FILE` — custom CLI config file
  - `TF_DATA_DIR` — override default data directory
  - Backend configuration: provide backend values via `-backend-config` or `backend` block in Terraform configuration; strata can pass `backend_config` to `terraform init`.

Example — passing backend config through strata (integration call):
