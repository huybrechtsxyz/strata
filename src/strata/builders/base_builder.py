...
class BaseBuilder(ABC):
    ...
    def _extract_source_at_ref(
        ...
        (e.g., repo_root is not a git repository). Shared by every provisioner-source
        copying builder (Terraform, Ansible, Bicep, ...) — ``SourceModel.reference``
        (ADR-0071) is a generic, provisioner-agnostic field ("only valid for
        git-based sources"), not a Terraform-specific one, so this helper lives on
        ``BaseBuilder`` rather than being duplicated or Terraform-exclusive.
