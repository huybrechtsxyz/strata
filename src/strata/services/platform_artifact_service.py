"""Service for managing platform model I/O operations (read/write PlatformModel artifacts)."""
...
class PlatformService(BaseService["PlatformArtifactModel"]):
    ...
    def load_from_json(self, path: Path) -> PlatformArtifactModel:
        ...
        """
        if self.verbose:
            self.logger.info("Loading platform model from JSON", path=str(path))
    ...
    def load_from_yaml(self, path: Path) -> PlatformArtifactModel:
        ...
        """
        if self.verbose:
            self.logger.info("Loading platform model from YAML", path=str(path))
    ...
    def save_to_json(self, platform: PlatformArtifactModel, path: Path, indent: int = 2) -> None:
        ...
        """
        if self.verbose:
            self.logger.info("Saving platform model to JSON", path=str(path))
    ...
    def save_to_yaml(self, platform: PlatformArtifactModel, path: Path) -> None:
        ...
        """
        if self.verbose:
            self.logger.info("Saving platform model to YAML", path=str(path))
