"""Controller for workspace logging configuration (`.strata/logging.yaml`)."""
...
class LoggingController(BaseController):
    """Manage reading and writing of `<work_path>/.strata/logging.yaml`.
    ...
    def set_value(self, key: str, value: str) -> Tuple[bool, List[str]]:
        ...
        level and the ``strata`` logger level in one operation.
        """
        ok, data = self.load()
        if not ok:
            return False, self.get_errors()
