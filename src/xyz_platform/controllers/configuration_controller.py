...
(`<work_path>/.platform/cli.yaml`) using the package template when available.
"""
...
from xyz_platform.utils.system import get_pkg_templates_path


class ConfigurationController(BaseController):
    """Manage reading and writing of `<work_path>/.platform/cli.yaml`.

    Responsibilities:
    - Load existing YAML config (returns dict)
    - Initialise from template when missing
    - Set/unset keys in the top-level `defaults:` mapping
    """
    ...
    def set_default(self, key: str, value: Any) -> Tuple[bool, List[str]]:
        ok, cfg = self.load_config()
        if not ok:
            cfg = {}
    ...
    def unset_default(self, key: str) -> Tuple[bool, List[str]]:
        ok, cfg = self.load_config()
        if not ok:
            return False, self._errors
        defaults = cfg.get("defaults", {}) or {}
