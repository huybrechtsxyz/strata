...
"""
===============================================================================
Script Name   : module_model.py
Author        : Vincent Huybrechts
Version       : 1.0.0
Python Version: 3.12+
Description   : Pydantic models for validating module configurations and parsing YAML definitions.
===============================================================================
"""
...
from xyz_platform.models.common_models import SourceModel
...
class ModuleReferenceModel(BaseModel):
    ...
    variables: VariableRefs = Field(
        None, description="List of variable keys this module requires from environment"
    )
    secrets: SecretRefs = Field(
        None, description="List of secret keys this module requires from environment"
    )
...
class ModuleEndpointModel(BaseModel):
    ...
    protocol: Optional[str] = Field(
        None, description="Protocol for the endpoint (e.g., tcp, udp)"
    )
...
class ModuleCheckModel(BaseModel):
    ...
    target: Optional[str] = Field(
        None, description="Target resource for the health check"
    )
    type: Optional[str] = Field(
        None, description="Type of health check (e.g., http, tcp, command)"
    )
    interval: Optional[str] = Field(
        None, description="Interval between health checks (e.g., '30s')"
    )
    timeout: Optional[str] = Field(
        None, description="Timeout for each health check (e.g., '5s')"
    )
    retries: Optional[int] = Field(
        None, description="Number of retries before marking as unhealthy"
    )
    command: Optional[List[str]] = Field(
        None, description="Command to run for 'command' type health checks"
    )
...
class ModuleMountModel(BaseModel):
    ...
    change_mod: Optional[str] = Field(
        None, description="Permissions for the mount (e.g., '755')"
    )
...
class ModulePropertiesModel(BaseModel):
    ...
    mounts: Optional[List[ModuleMountModel]] = Field(
        None, description="List of module mount configurations"
    )
    checks: Optional[List[ModuleCheckModel]] = Field(
        None, description="List of module health check configurations"
    )
    endpoints: Optional[List[ModuleEndpointModel]] = Field(
        None, description="List of module endpoint configurations"
    )
...
class ModuleMetaModel(BaseModel):
    ...
    tags: Optional[List[Any]] = Field(
        None, description="Optional list of tags for the module"
    )
...
class ModuleModel(BaseModel):
    ...
    meta: ModuleMetaModel = Field(
        description="Module metadata (name, annotations, labels, tags)"
    )
    spec: ModuleSpecModel = Field(
        description="Module specification (lifecycle, modules, variables, secrets)"
    )
