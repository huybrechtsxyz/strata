...
class TerraformDeployer(BaseDeployer):
    ...
    def output(self) -> Tuple[bool, Dict[str, Any], List[str]]:
        ...
        try:
            ...
            resolved = {k: v.get("value") for k, v in raw.items() if isinstance(v, dict)}
    ...
    def _build_working_dir(self) -> Path:
        ...
        target = self._iac_model.source.target_path or f"terraform/{self._iac_model.name}"
    ...
    def _make_tf_integration(name: str) -> TerraformIntegration:
        ...
        return TerraformIntegration(config=IntegrationModel(name=name, type="terraform"))
    ...
    def _resolve_iac_model(
        ...
        if stage.topology:
            topo = next(
                (t for t in (spec.topology or []) if t.name == stage.topology), None
            )
            if topo:
                match = next(
                    (p for p in provisioners if p.provisioner == topo.provisioner), None
                )
                if match:
                    return match
                logger.warning(
        ...
        return None
                ...
                Execution steps per stage:
                  1. before_deploy  — verify working directory and artefacts exist
                  2. deploy         — terraform init → validate → plan (→ apply)
                  3. after_deploy   — verify plan/state artefacts

                Dry-run behaviour:
                  Steps 1-3 are executed through plan only.  apply is skipped.
                  All output is prefixed with [DRY-RUN].

                Working-directory resolution:
                  stage.provisioner (name)
                    → workspace.spec.provisioners[name == stage.provisioner]
                  OR
                  stage.topology (name)
                    → workspace.spec.topology[name == stage.topology].provisioner
                    → workspace.spec.provisioners[.provisioner == topo.provisioner]

                  working_dir = deployment_build_path / iac_model.source.target_path

                TODO (destroy): terraform destroy is not yet implemented.
                TODO (rollback): on_failure=rollback is not yet implemented.
===============================================================================
"""
