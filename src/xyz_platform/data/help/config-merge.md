...
stored in memory as `_merged_config` and written to `.platform/configuration.yaml`
...
After any command runs, check `.platform/configuration.yaml` to see what was
actually merged. This file is overwritten on every run — it reflects the last
command's active profile at that moment.

```
cat .platform/configuration.yaml    # Unix
Get-Content .platform\configuration.yaml    # PowerShell
```
