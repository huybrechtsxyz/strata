...
"""
===============================================================================
Script Name   : integration_service.py
Author        : Vincent Huybrechts
Version       : 1.0.0
Python Version: 3.12+
Description   : Service for managing platform integrations (Centralized Singleton pattern)

IntegrationService is a manager service that coordinates integration loading,
registration, and access. It bridges the configuration layer and runtime
integration instances.

Responsibilities:
- Load integration specs from ConfigurationService
- Create integration instances via IntegrationFactory
- Register integrations in IntegrationRegistry
- Validate required integrations
- Provide integration access to other services
...
class IntegrationService:
    ...
    def initialize_integrations(
        ...
        logger.info("Loading integrations", extra={"count": len(integration_specs)})
        ...
        for spec in integration_specs:
            ...
            try:
                ...
                if spec.required:
                    ...
                    logger.warning(f"Optional integration failed: {error_msg}")

        logger.info(
            "Integration initialization complete",
            extra={"loaded": loaded_count, "total": len(integration_specs)},
        )
    ...
    def validate_required_integrations(self) -> Tuple[bool, List[str]]:
        ...
        logger.debug(
            "Checking required integrations",
            extra={"count": len(required_integrations)},
        )
        ...
        if errors:
            logger.warning(
                "Required integration validation failed",
    ...
    def get_integration_with_capability(self, capability: Type):
        ...
        if not integrations:
            logger.warning(
                "No integrations found with capability",
                extra={"capability": capability.__name__},
            )
            return None
        ...
        logger.warning(
            "No available integrations found with capability",
            extra={"capability": capability.__name__, "checked": len(integrations)},
        )
        return None
