"""
Loads workspace-local integration drop-ins from .platform/integrations/*.py.
...
def load_workspace_integrations(work_path: Path) -> int:
    """
    Scan ``.platform/integrations/*.py`` and call ``register()`` in each.
    ...
    Args:
        work_path: Root of the workspace (the directory that contains ``.platform/``).

    Returns:
        Number of files successfully loaded (i.e. ``register()`` was called
        without raising).
    """
    integrations_dir = work_path / ".platform" / "integrations"
