...
"""
===============================================================================
Script Name   : add_config_session_command.py
...
class AddConfigSessionCommand(BaseCommand):
    ...
    def __init__(
        ...
        Args:
            ...
            config_file: URL or path to the file
    ...
    def execute(self) -> bool:
        ...
        try:
            ...
            # Add repository
            success, self._added_configs = self._session_controller.add_config_source(
    ...
    def _initialize(
        ...
        if not self._item_path:
            error_msg = "--config-file (for a config source) is required"
            self.logger.error(error_msg)
            self._errors.append(error_msg)
            return False

        self.logger.debug(
            "Add session config command initializing",
            extra={
                "command_class": self.__class__.__name__,
                "config_name": self._item_name,
                "config_path": self._item_path,
    ...
    def _before_execute(self) -> bool:
        ...
        self.logger.debug(
            "Add session config pre-execution validating",
            extra={
                "command_class": self.__class__.__name__,
                "config_name": self._item_name,
            },
    ...
    def _after_execute(self) -> bool:
        ...
        """
        self.logger.debug(
            "Add session config command post-executing",
            extra={
                "command_class": self.__class__.__name__,
                "config_name": self._item_name,
            },
        ...
        if not self._is_quiet() and self._added_configs:
            # Always populate structured output data
            self._output_data = {
                k: v for k, v in self._added_configs.items() if v is not None
            }

            if self._is_console_output():
                click.echo("\n📦  Added item:")
                if self._added_configs.get("name"):
                    click.echo(f"    • Name:   {self._added_configs['name']}")
                if self._added_configs.get("path"):
                    click.echo(f"    • Path:   {self._added_configs['path']}")
                click.echo("")
    ...
    def _finalize(self, success: bool = False, show_footer: bool = True) -> bool:
        ...
        """
        self.logger.debug(
            "Add session config command finalizing",
            extra={
                "command_class": self.__class__.__name__,
                "config_name": self._item_name,
            },
