...
def _collect_available_templates(work_path: Optional[Path]) -> list[str]:
    ...
    Workspace templates (`.platform/templates/`) take precedence but both
    ...
    if work_path is not None:
        ws_dir = work_path / ".platform" / "templates"
...
def _resolve_template_path(template: str, work_path: Optional[Path]) -> Optional[Path]:
    ...
    if work_path is not None:
        ws_path = work_path / ".platform" / "templates" / f"{template}.yaml"
        if ws_path.exists():
            return ws_path
