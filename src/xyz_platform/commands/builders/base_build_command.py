...
class BaseBuildCommand(BaseCommand):
    ...
    def _load_configuration_service(self) -> Optional[ConfigurationService]:
        ...
        if self._solution_controller.solution is None:
            self._errors.append("Build requires an initialized workspace. Run `xyz solution init` first.")
            return None

        profile, _ = self._solution_controller.get_active_profile()
        if profile is None:
            self._errors.append("Build requires an active profile. Run `xyz profile activate <name>`.")
            return None
