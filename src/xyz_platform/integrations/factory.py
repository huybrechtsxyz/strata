...
Version       : 1.0.0
Python Version: 3.12+
Description   : Factory for creating integration instances from configuration.

IntegrationFactory maps integration types to concrete integration classes
and creates instances based on IntegrationModel configuration.
...
Usage:
    ...
    integration = factory.create(integration_spec)
===============================================================================
"""
...
class IntegrationFactory:
    ...
    def register_type(
        ...
        logger.debug(
            "Integration type registered",
            extra={"type": integration_type, "class": integration_class.__name__},
        )
    ...
    def unregister_type(cls, integration_type: str):
        ...
        if integration_type in cls._type_mapping:
            ...
            logger.debug(
                "Integration type unregistered", extra={"type": integration_type}
            )
...
def _auto_register_builtin_integrations():
    ...
    for integration_type, module_path, class_name in builtin_integrations:
        try:
            ...
            logger.debug(
                "Built-in integration auto-registered",
                extra={"type": integration_type, "class": class_name},
            )

        except ImportError as e:
            logger.debug(
                "Built-in integration not available (not yet implemented)",
                extra={"type": integration_type, "error": str(e)},
            )
        except Exception as e:
            logger.warning(
                "Failed to auto-register built-in integration",
                extra={"type": integration_type, "error": str(e)},
                exc_info=True,
            )
