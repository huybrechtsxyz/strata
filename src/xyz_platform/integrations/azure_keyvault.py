...
"""
===============================================================================
Script Name   : azure_keyvault.py
Author        : Vincent Huybrechts
Version       : 1.0.0
Python Version: 3.12+
Description   : Azure Key Vault integration for XYZ Platform.

Azure Key Vault integration for secret storage operations.
...
class AzureKeyVaultIntegration(StoreIntegration):
    ...
    def get_secret(
        ...
        if not available:
            logger.warning(
                "Cannot retrieve secret from Azure Key Vault",
        ...
        if prefer_cli:
            ...
            if result:
                logger.info(
                    ...
                    extra={"secret_name": key, "name": self.integration_name},
                )
                return result
    ...
    def list_secrets(self, prefix: str = "", **kwargs) -> List[str]:
        ...
        if not available:
            logger.debug(
                "Cannot list secrets from Azure Key Vault",
                extra={"error": error, "name": self.integration_name},
            )
            return []
    ...
    def _get_access_token_via_api(self) -> Optional[str]:
        ...
        try:
            logger.debug(
                "Authenticating to Azure Key Vault using client credentials",
                extra={"name": self.integration_name},
            )
            token_url = (
