...
- **Diagram Builder + AI generation (ADR-0034, Phase 4)** — `Strata: New Diagram (Builder)` and per-entry "Edit in Diagram Builder" open a webview form (sources, layout, style) that live-previews through the existing preview pane and validates before saving to `.strata/diagrams/`; a hand-written-template diagram is detected and declined with a pointer to `--print-template`. `@strata /diagram create <description>` generates a definition from natural language (validated with a one-retry repair loop, never a silently-broken diagram) and opens it in the Builder. "Copy Mermaid" exports the rendered source to the clipboard; "Export SVG"/"Export PNG" produce real image files entirely client-side (the preview webview serializes/rasterizes what it already rendered — no network call, no Kroki needed for this path).
