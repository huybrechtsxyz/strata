...
class TestAuditList:
    def test_list_basic(self, tmp_path):
        runner = CliRunner()
        with patch("strata.commands.audit.show_audit_command.ShowAuditCommand.execute", return_value=True):
    ...
    def test_list_execute_false_returns_nonzero(self, tmp_path):
        runner = CliRunner()
        with patch("strata.commands.audit.show_audit_command.ShowAuditCommand.execute", return_value=False):
            result = runner.invoke(audit_group, ["list", "--work-path", str(tmp_path)])
        assert result.exit_code != 0
