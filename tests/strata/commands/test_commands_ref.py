...
class TestRefRemove:
    def test_remove_basic(self, tmp_path):
        runner = CliRunner()
        with patch(
            "strata.commands.ref.remove_profile_path_command.RemoveProfilePathCommand.execute", return_value=True
        ):
            result = runner.invoke(
                ref_group,
...
class TestRefList:
    def test_list_basic(self, tmp_path):
        runner = CliRunner()
        with patch("strata.commands.cli_ref._ListSingleTypeCommand.execute", return_value=True):
            result = runner.invoke(
                ref_group,
...
class TestRefShow:
    def test_show_basic(self, tmp_path):
        runner = CliRunner()
        with patch("strata.commands.ref.show_ref_command.ShowRefCommand.execute", return_value=True):
            result = runner.invoke(
                ref_group,
