...
class TestProfileAdd:
    def test_add_basic(self, tmp_path):
        runner = CliRunner()
        with patch("xyz_platform.commands.profile.add_profile_command.AddProfileCommand.execute", return_value=True):
    ...
    def test_add_execute_false_returns_nonzero(self, tmp_path):
        runner = CliRunner()
        with patch("xyz_platform.commands.profile.add_profile_command.AddProfileCommand.execute", return_value=False):
            result = runner.invoke(profile_group, ["add", "staging", "--work-path", str(tmp_path)])
        assert result.exit_code != 0
...
class TestProfileRemove:
    def test_remove_basic(self, tmp_path):
        runner = CliRunner()
        with patch(
            "xyz_platform.commands.profile.remove_profile_command.RemoveProfileCommand.execute", return_value=True
        ):
...
class TestProfileList:
    def test_list_basic(self, tmp_path):
        runner = CliRunner()
        with patch("xyz_platform.commands.profile.list_profile_command.ListProfileCommand.execute", return_value=True):
...
class TestProfileActivate:
    def test_activate_basic(self, tmp_path):
        runner = CliRunner()
        with patch(
            "xyz_platform.commands.profile.activate_profile_command.ActivateProfileCommand.execute", return_value=True
        ):
...
class TestProfileShow:
    def test_show_basic(self, tmp_path):
        runner = CliRunner()
        with patch(
            "xyz_platform.commands.ref.list_profile_path_command.ListProfilePathCommand.execute", return_value=True
        ):
