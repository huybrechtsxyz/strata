...
class TestDeployRun:
    def test_run_basic(self, tmp_path):
        runner = CliRunner()
        with patch("xyz_platform.commands.deploy.run_deploy_command.RunDeployCommand.execute", return_value=True):
    ...
    def test_run_execute_false_returns_nonzero(self, tmp_path):
        runner = CliRunner()
        with patch("xyz_platform.commands.deploy.run_deploy_command.RunDeployCommand.execute", return_value=False):
            result = runner.invoke(deploy, ["run", "--work-path", str(tmp_path)])
        assert result.exit_code != 0
...
class TestDeployDestroy:
    def test_destroy_dry_run(self, tmp_path):
        runner = CliRunner()
        with patch(
            "xyz_platform.commands.deploy.destroy_deploy_command.DestroyDeployCommand.execute", return_value=True
        ):
    ...
    def test_destroy_execute_false_returns_nonzero(self, tmp_path):
        runner = CliRunner()
        with patch(
            "xyz_platform.commands.deploy.destroy_deploy_command.DestroyDeployCommand.execute", return_value=False
        ):
            result = runner.invoke(deploy, ["destroy", "--dry-run", "--work-path", str(tmp_path)])
        assert result.exit_code != 0
...
class TestDeployStatus:
    def test_status_basic(self, tmp_path):
        runner = CliRunner()
        with patch("xyz_platform.commands.deploy.status_deploy_command.StatusDeployCommand.execute", return_value=True):
...
class TestDeployHistory:
    def test_history_basic(self, tmp_path):
        runner = CliRunner()
        with patch(
            "xyz_platform.commands.deploy.history_deploy_command.HistoryDeployCommand.execute", return_value=True
        ):
