...
class TestValuesList:
    ...
    def test_basic_list_mocked(self, tmp_path):
        runner = CliRunner()
        with patch(
            "xyz_platform.commands.deploy.list_values_deploy_command.ListValuesDeployCommand.execute",
            return_value=True,
        ):
...
class TestValuesGet:
    ...
    def test_single_key_mocked(self, tmp_path):
        runner = CliRunner()
        with patch(
            "xyz_platform.commands.deploy.get_values_deploy_command.GetValuesDeployCommand.execute",
            return_value=True,
        ):
            result = runner.invoke(
