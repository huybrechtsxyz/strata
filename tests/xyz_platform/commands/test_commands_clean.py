...
class TestCleanCommand:
    def test_basic_invocation_mocked(self, tmp_path):
        runner = CliRunner()
        with patch(
            "xyz_platform.commands.clean.clean_solution_command.CleanSolutionCommand.execute", return_value=True
        ):
    ...
    def test_execute_false_returns_nonzero(self, tmp_path):
        runner = CliRunner()
        with patch(
            "xyz_platform.commands.clean.clean_solution_command.CleanSolutionCommand.execute", return_value=False
        ):
            result = runner.invoke(clean_command, ["--work-path", str(tmp_path)])
        assert result.exit_code != 0
