...
class TestInitCommand:
    ...
    def test_name_option_accepted(self, tmp_path):
        runner = CliRunner()
        with patch("xyz_platform.commands.init.init_solution_command.InitSolutionCommand.execute", return_value=True):
    ...
    def test_execute_false_returns_nonzero(self, tmp_path):
        runner = CliRunner()
        with patch("xyz_platform.commands.init.init_solution_command.InitSolutionCommand.execute", return_value=False):
            result = runner.invoke(init_command, ["--name", "myapp", "--work-path", str(tmp_path)])
        assert result.exit_code != 0
