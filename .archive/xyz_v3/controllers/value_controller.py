...
"""
===============================================================================
Script Name   : value_controller.py
Author        : XYZ Platform Team
Version       : 1.0.0
Python Version: 3.12+
Description   : Resolves variables, secrets, and feature flags from their
                configured store backends into concrete runtime values.

                Resolution is driven by the merged EnvironmentService attached
                to the deployment.  Each item carries a 'store' type that
                selects the resolution strategy:

                  constant    — value is returned as-is (literal)
                  environment — value is read from os.environ
                  <type>      — delegated to the registered integration whose
                                integration_type matches the store string
                                (e.g. "azure-keyvault", "consul", "vault", …)

                Result is a ResolvedValues dataclass that also exposes the
                values as a TF_VAR_* dict ready for Terraform injection.

                Errors are non-fatal by default (missing optional values are
                skipped with a warning); pass strict=True to treat any
                resolution failure as an error.
===============================================================================
"""
