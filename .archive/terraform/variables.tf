...
# Workspace
variable "workspace" {
  description = "Workspace that is being created"
  type        = string
}
...
# Private key for SSH access to the servers
variable "kamatera_private_key" {
  description = "Private key for SSH access to the servers"
  type        = string
  sensitive   = true
}
...
variable "virtualmachines" {
  ...
  validation {
    condition     = alltrue([for r in var.virtualmachines : r.provider == "kamatera"])
    error_message = "Provider for virtual machines must be 'kamatera'."
  }
